...
async function runStatsUpdater(client) {
    ...
    try {
        ...
        if (!allSettings || !Array.isArray(allSettings) || allSettings.length === 0) {
            logger.info('📊 No server settings found. Skipping update cycle.');
            return;
        }

        const activeSettings = allSettings.filter((s) => s && guildsCache.has(s.guildId) && s.serverStatsOn);
