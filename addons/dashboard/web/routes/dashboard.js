...
router.get('/dashboard/:guildId', isAuthorized, checkServerAccess, async (req, res) => {
    let recentLogs = [];
    try {
        if (ModLog && ModLog.getAllCache && typeof ModLog.getAllCache === 'function') {
            try {
                ...
                if (logsFromDb && Array.isArray(logsFromDb) && logsFromDb.length > 0) {
                    recentLogs = logsFromDb.map((log) => ({
                        moderator: log && log.moderatorTag ? log.moderatorTag : 'Unknown',
                        target: log && log.targetTag ? log.targetTag : 'Unknown',
                        action: log && log.action ? log.action : 'Unknown',
                        reason: (log && log.reason) || 'Tidak ada alasan',
                    }));
                }
            } catch (dbError) {
    ...
    renderDash(res, 'dashboard', {
        ...
        stats: {
            ...
            channelCount: (req.guild.channels && req.guild.channels.cache) ? req.guild.channels.cache.size : 0,
            roleCount: (req.guild.roles && req.guild.roles.cache) ? req.guild.roles.cache.size : 0,
        },
        currentPage: '/dashboard',
        logs: recentLogs,
        page: 'dashboard',
...
router.post('/dashboard/:guildId/settings', isAuthorized, checkServerAccess, async (req, res) => {
    try {
        ...
        if (!settings || !ServerSetting || typeof ServerSetting.getAttributes !== 'function') {
            throw new Error('Settings model is not available');
        }
        ...
        for (const key of settingKeys) {
            ...
            if (attribute && attribute.type && attribute.type.key === 'BOOLEAN') {
                settings[key] = value === 'on';
            } else if (attribute && attribute.type && attribute.type.key === 'JSON') {
