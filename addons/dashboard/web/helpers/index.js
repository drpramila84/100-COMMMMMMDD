    ...
    try {
        ...
        if (!guild) {
            return res.status(404).render('error', {
                ...
                message: 'The bot is not in this server or the server ID is invalid.',
                page: '/',
                currentPage: '',
                user: req.user || null,
                guild: null,
        ...
        let settings = await ServerSetting.getCache({ guildId: guild.id }).catch(() => null);
        if (!settings) {
            try {
                await ServerSetting.create({ guildId: guild.id, guildName: guild.name }).catch(() => null);
                settings = await ServerSetting.getCache({ guildId: guild.id }).catch(() => null);
            } catch (e) {
                logger.error(`Failed to create settings for guild ${guildId}:`, e);
                settings = {};
            }
        }
        ...
        console.error('Error in checkServerAccess middleware:', error);

        return res.status(500).render('error', {
            statusCode: 500,
            title: 'Server Error',
            message: 'An error occurred while verifying server access. Please try again later.',
            page: '/',
            currentPage: '',
            user: req.user || null,
            guild: null,
